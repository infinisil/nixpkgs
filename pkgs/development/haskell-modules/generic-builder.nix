...
, doInstallIntermediates ? false, editedCabalFile ? null
...
enableLibraryForGhci ? false
  ...
  # this build
, previousIntermediates ? null }@args:
...
in lib.fix (drv:
  ...
  stdenv.mkDerivation ({
    ...
    ''
      ...
      + optionalString (!stdenv.buildPlatform.isDarwin
    ...
    ${if doInstallIntermediates then "installIntermediatesPhase" else null} = ''
      ...
      intermediatesOutput=${
        if enableSeparateIntermediatesOutput then "$intermediates" else "$out"
      }
