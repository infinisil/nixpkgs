...
let
  ...
  packages = ql.overrideScope' (self: super: {
    ...
    cl-unicode = build-with-compile-into-pwd {
      ...
      src = pkgs.fetchzip {
        url =
          "https://github.com/edicl/cl-unicode/archive/refs/tags/v0.1.6.tar.gz";
    ...
    cl-tar-file = build-asdf-system {
      ...
      version = "v0.2.1";
      src = pkgs.fetchzip {
        url = let rev = "0c10bc82f14702c97a26dc25ce075b5d3a2347d1";
    ...
    cl-tar = build-asdf-system {
      ...
      version = "v0.2.1";
      src = pkgs.fetchzip {
        url = let rev = "7c6e07a10c93d9e311f087b5f6328cddd481669a";
    ...
    facts = build-asdf-system {
      ...
      src = pkgs.fetchzip {
        url =
          "https://beta.quicklisp.org/archive/cl-facts/2022-11-06/cl-facts-20221106-git.tgz";
    ...
    qt = let rev = "dffff3ee3dbd0686c85c323f579b8bbf4881e60e";
