...
in lib.makeOverridable ({
  ...
  in assert lib.versionOlder version "5.8" -> libelf != null;
  ...
  stdenv.mkDerivation ({
    ...
    installTargets = [
      (kernelConf.installTarget or (if target == "uImage" then
        "uinstall"
      else if target == "zImage" || target == "Image.gz" then
        "zinstall"
      else
        "install"))
    ];
