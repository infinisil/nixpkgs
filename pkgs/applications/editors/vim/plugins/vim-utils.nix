...
{ lib, stdenv, vim, vimPlugins, buildEnv, writeText, runCommand, makeWrapper
, python3, callPackage, makeSetupHook, linkFarm }:
...
let
  ...
  */
  vimrcContent = { packages ? null, vam ? null, # deprecated
    pathogen ? null, # deprecated
    plug ? null, beforePlugins ? ''
...
in rec {
  ...
  makeCustomizable =
    ...
    vim // {
      ...
      customize = { # The name of the derivation.
        ...
        wrapManual ? null, wrapGui ? null, vimExecutableName ? null
  ...
  requiredPluginsForPackage = { start ? [ ], opt ? [ ] }: start ++ opt;
