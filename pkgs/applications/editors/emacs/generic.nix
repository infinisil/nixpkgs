{ pname, version, variant, src, patches ? _: [ ] }:

{ lib, stdenv, Xaw3d, acl, alsa-lib, autoreconfHook, cairo, dbus
, emacsPackagesFor, fetchpatch, gconf, gettext, giflib, glib-networking, gnutls
, gpm, gsettings-desktop-schemas, gtk2-x11, gtk3, gtk3-x11, harfbuzz
, imagemagick, jansson, libXaw, libXcursor, libXft, libXpm, libgccjit, libjpeg
, libotf, libpng, librsvg, libselinux, libtiff, libwebp, libxml2, llvmPackages_6
, m17n_lib, makeWrapper, motif, ncurses, nixosTests, pkg-config
, recurseIntoAttrs, sigtool, sqlite, substituteAll, systemd, tree-sitter
, texinfo, webkitgtk, wrapGAppsHook

# macOS dependencies for NS and macPort
, AppKit, Carbon, Cocoa, GSS, IOKit, ImageCaptureCore, ImageIO, OSAKit, Quartz
, QuartzCore, WebKit

# Boolean flags
, nativeComp ? null, withNativeCompilation ? if nativeComp != null then
  lib.warn
  "nativeComp option is deprecated and will be removed; use withNativeCompilation instead"
  nativeComp
else
  true, noGui ? false, srcRepo ? true, withAcl ? false, withAlsaLib ? false
, withAthena ? false, withCsrc ? true, withGTK2 ? false
, withGTK3 ? withPgtk && !noGui, withGconf ? false, withGpm ? stdenv.isLinux
, withImageMagick ? lib.versionOlder version "27" && (withX || withNS)
, withMotif ? false
, withNS ? stdenv.isDarwin && !(variant == "macport" || noGui), withPgtk ? false
, withSQLite3 ? lib.versionAtLeast version "29"
, withSystemd ? lib.meta.availableOn stdenv.hostPlatform systemd
, withToolkitScrollBars ? true, withTreeSitter ? lib.versionAtLeast version "29"
, withWebP ? lib.versionAtLeast version "29"
, withX ? !(stdenv.isDarwin || noGui || withPgtk)
, withXinput2 ? withX && lib.versionAtLeast version "29", withXwidgets ? !noGui
  && (withGTK3 || withPgtk)

  # Options
, siteStart ? ./site-start.el, toolkit ? (if withGTK2 then
  "gtk2"
...
in mkDerivation (finalAttrs:
  (lib.optionalAttrs withNativeCompilation {
    ...
    buildInputs = [ gettext gnutls harfbuzz.dev jansson libxml2 ncurses ]
      ++ lib.optionals withGconf [ gconf ] ++ lib.optionals withAcl [ acl ]
      ++ lib.optionals withAlsaLib [ alsa-lib ] ++ lib.optionals withGpm [ gpm ]
      ++ lib.optionals stdenv.isLinux [ dbus libselinux ]
      ++ lib.optionals (!stdenv.isDarwin && withGTK3)
      [ gsettings-desktop-schemas ]
      ++ lib.optionals (stdenv.isLinux && withX) [ libotf m17n_lib ]
      ++ lib.optionals (withX && withGTK2) [ gtk2-x11 ]
      ++ lib.optionals (withX && withGTK3) [ gtk3-x11 ]
      ++ lib.optionals (withX && withMotif) [ motif ]
      ++ lib.optionals (withX && withXwidgets) [ glib-networking ]
      ...
      ++ lib.optionals withWebP [ libwebp ] ++ lib.optionals withX [
        Xaw3d
        cairo
        giflib
        libXaw
      ...
      ++ lib.optionals withNS [ librsvg AppKit GSS ImageIO ]
    ...
    '' + lib.optionalString
      (withNativeCompilation && (withNS || variant == "macport")) ''
    ...
    meta = {
      homepage = if variant == "macport" then
      ...
      platforms = if variant == "macport" then
