...
let
  ...
  combine = import ./combine.nix {
    inherit bin combinePkgs buildEnv lib makeWrapper writeText runCommand stdenv
      python3 ruby perl gnused gnugrep coreutils libfaketime makeFontsConf;
    ghostscript = ghostscript_headless;
  };
  ...
  tl = let
    ...
    overridden = orig // {
      ...
      texlive-msg-translations =
        builtins.removeAttrs orig.texlive-msg-translations [ "hasTlpkg" ];
    ...
    let
      ...
      mkPkgV = tlType:
        let
          pkg = attrs // {
            sha512 =
              attrs.sha512.${if tlType == "tlpkg" then "run" else tlType};
  ...
  mkFixedName = { tlType, revision, extraRevision ? "", ... }@attrs:
    mkURLName attrs + (lib.optionalString (tlType == "tlpkg") ".tlpkg")
  ...
  mkPkg = { pname, tlType, revision, version, sha512, extraRevision ? ""
    , postUnpack ? "", stripPrefix ? 1, ... }@args:
    let
      # the basename used by upstream (without ".tar.xz" suffix)
      urlName = mkURLName args;
      tlName = mkTLName args;
      fixedHash = fixedHashes.${
          mkFixedName args
        } or null; # be graceful about missing hashes
  ...
  assertions = with lib;
    ...
    && (!useFixedHashes || (let
      all = concatLists (catAttrs "pkgs" (attrValues tl));
      fods = filter (p: isDerivation p && p.tlType != "bin") all;
    in builtins.all (p:
      assertMsg (p ? outputHash) "The TeX Live package '${
        p.pname + lib.optionalString (p.tlType != "run") ("." + p.tlType)
      }' does not have a fixed output hash. Please read UPGRADING.md on how to build a new 'fixed-hashes.nix'.")
    fods));
