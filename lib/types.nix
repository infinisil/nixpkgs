    ...
    types = rec {
      ...
      package = mkOptionType {
        ...
        merge = loc: defs:
          ...
          in if builtins.isPath res
          || (builtins.isString res && !builtins.hasContext res) then
            toDerivation res
          else
            res;
      };
      ...
      nullOr = elemType:
        mkOptionType rec {
          ...
          merge = loc: defs:
            let nrNulls = count (def: def.value == null) defs;
      ...
      functionTo = elemType:
        mkOptionType {
          ...
          merge = loc: defs: fnArgs:
            (mergeDefinitions (loc ++ [ "<function body>" ]) elemType (map
              (fn: {
                inherit (fn) file;
      ...
      submoduleWith = { modules, specialArgs ? { }
        , shorthandOnlyDefinesConfig ? false, description ? null, class ? null
        }@attrs:
        let
        ...
        in mkOptionType {
          ...
          functor = defaultFunctor name // {
            ...
            payload = {
              inherit modules class specialArgs shorthandOnlyDefinesConfig
                description;
            };
            binOp = lhs: rhs: {
              class =
      ...
      either = t1: t2:
        mkOptionType rec {
          ...
          merge = loc: defs:
            let defList = map (d: d.value) defs;
      ...
      coercedTo = coercedType: coerceFunc: finalType:
        ...
        mkOptionType rec {
          ...
          merge = loc: defs:
            let
              coerceVal = val:
                if coercedType.check val then coerceFunc val else val;
            in finalType.merge loc
