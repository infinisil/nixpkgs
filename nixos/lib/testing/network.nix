...
let
  ...
  networkModule = { config, nodes, pkgs, ... }:
    let
      ...
      vlansNumbered =
        forEach (zipLists config.virtualisation.vlans (range 1 255)) (v: {
          name = "eth${toString v.snd}";
          vlan = v.fst;
          assignIP = true;
        });
      explicitInterfaces = lib.mapAttrsToList (n: v: v // { name = n; })
      ...
      qemuOptions = lib.flatten (forEach interfacesNumbered ({ fst, snd }:
        qemu-common.qemuNICFlags snd fst.vlan
      ...
      udevRules = forEach interfacesNumbered ({ fst, snd }:
        ...
        ''
          SUBSYSTEM=="net",ACTION=="add",ATTR{address}=="${
            toLower (qemu-common.qemuNicMac fst.vlan
              config.virtualisation.test.nodeNumber)
          }",NAME="${fst.name}"'');
      ...
      networkConfig = {
        ...
        networking.primaryIPAddress = optionalString (ipInterfaces != [ ])
