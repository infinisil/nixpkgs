...
let
  ...
  inherit (lib)
      attrValues concatStringsSep filterAttrs length listToAttrs
      literalExpression makeSearchPathOutput mkEnableOption mkIf mkOption
      nameValuePair optionals types;
  ...
  config = let
    ...
    mkInstanceService = instance: {
      ...
      script = let
        ...
        outputPipeline =
          [ "appsrc name=appsrc ${appsrcOptions}" "videoconvert" ]
