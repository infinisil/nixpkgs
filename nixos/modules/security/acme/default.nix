...
let
  ...
  certToConfig = cert: data:
    let
      ...
      renewOpts = escapeShellArgs (commonOpts ++ [ "renew" "--no-random-sleep" ]
        ++ optionals data.ocspMustStaple [ "--must-staple" ]
    ...
    in {
      ...
      renewService = {
        ...
        serviceConfig = commonServiceConfig // {
          ...
          && toInt (last (splitString ":" data.listenHTTP)) < 1024) {
    ...
    in {
      options = {
        validMinDays = mkOption {
          ...
          description =
            lib.mdDoc "Minimum remaining validity before renewal in days.";
        };
...
in {
  ...
  imports = [
    ...
    (mkChangedOptionModule [ "security" "acme" "email" ] [
      "security"
      "acme"
      "defaults"
      "email"
    ] (config: config.security.acme.email))
    (mkChangedOptionModule [ "security" "acme" "server" ] [
      "security"
      "acme"
      "defaults"
      "server"
